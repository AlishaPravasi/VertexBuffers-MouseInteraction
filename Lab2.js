...
Lab2.prototype.init = function () {
  ...
  //set up geometry "radio" buttons
//  var points = this.canvasID + "-" + "points";
//  var lines = this.canvasID + "-lines";
//  var lineStrips = this.canvasID + "-line-strips";
//  var lineLoops = this.canvasID + "-line-loops";
//  var triangles = this.canvasID + "-triangles";
//  var triangleFan = this.canvasID + "-triangle-fan";
//  var triangleStrips = this.canvasID + "-triangle-strips";
  
 
  this.radioButtons = document.querySelectorAll('input[name="geometry"]');
  ...
  canvas.addEventListener("mouseup", function (e) {
    ...
    t.vertexData.push(vec3(t.mX, t.mY, 0.0));
    t.vertexData.push(vec3(t.getSliderColor()));
    
    t.pointCount++;
    console.log(t.pointCount);
    
    let geometry;
    
    //this is probably the part that's broken
    for(const geo of t.radioButtons)
    {
      if(geo.checked)
      {
  ...
  canvas.addEventListener("mousemove", function (e) {
    ...
    t.mX = ((x-(t.canvas.width/2))/(t.canvas.width/2)).toFixed(3);
    t.mY = ((y-(t.canvas.height/2))/(t.canvas.height/(-2))).toFixed(3);
    
    clipX.textContent = t.mX;
    clipY.textContent = t.mY;
  });
...
Lab2.prototype.Render = function () {
  ...
  gl.drawArrays(gl.POINTS, 0, this.pointCount);
    ...
    //this is probably the part that's broken
    for(const geo of this.radioButtons)
    {
      if(geo.checked)
      {
          console.log(geo.value);
